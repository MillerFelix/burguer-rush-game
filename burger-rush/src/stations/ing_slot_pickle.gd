## Slot do Andar 4: Picles
## Responde ao [Clique do Mouse] para pegar/devolver fatias de picles.
...
func get_interaction_prompt(player: Node = null) -> String:
	...
	if not fridge:
		return ""
	return fridge.get_ingredient_prompt(player, "pickle")
...
func return_item(player: Node3D) -> void:
	var fridge = _get_fridge()
	if fridge:
		fridge.handle_ingredient_return(player, "pickle")
