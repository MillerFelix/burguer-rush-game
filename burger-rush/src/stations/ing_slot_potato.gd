## Slot do Andar 1: Sacos de Batata Frita Congelada
## Responde ao [Clique do Mouse] para pegar/devolver sacos de batata.
...
func get_interaction_prompt(player: Node = null) -> String:
	...
	if not fridge:
		return ""
	return fridge.get_ingredient_prompt(player, "potato_raw")
...
func return_item(player: Node3D) -> void:
	var fridge = _get_fridge()
	if fridge:
		fridge.handle_ingredient_return(player, "potato_raw")
