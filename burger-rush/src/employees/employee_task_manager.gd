...
# Ponto de descanso padrão na cozinha para funcionários livres (área reservada)
@export var default_rest_position: Vector3 = Vector3(-4.5, 0.0, -3.5)
...
func _cleanup_invalid_tasks() -> void:
	active_tasks = active_tasks.filter(func(t: EmployeeTask):
		...
		match t.task_type:
			...
			EmployeeTask.TaskType.CLEAN_TABLE:
				var table = t.target_node as RestaurantTable
				return table != null and table.table_state == RestaurantTable.TableState.DIRTY and table.seated_customers.is_empty()

			EmployeeTask.TaskType.CLEAN_PUDDLE, EmployeeTask.TaskType.CLEAN_FLOOR_SPOT:
				return is_instance_valid(t.target_node) and not t.target_node.is_queued_for_deletion()
...
func _scan_world_for_tasks() -> void:
	...
	for c in customers:
		...
		if cust and is_instance_valid(cust) and cust.state == Customer.State.SEATED_WAITING_TO_ORDER:
			...
			if table and not _has_active_task_for_target(table, EmployeeTask.TaskType.SERVE_CUSTOMER):
				var pos = table.global_position + Vector3(0.0, 0.0, 0.7)
	...
	if delivery_station and is_instance_valid(delivery_station):
		var deliv_mgr = tree.root.find_child("DeliveryQueueManager", true, false)
		if deliv_mgr and deliv_mgr.has_method("has_waiting_car_for_order") and deliv_mgr.has_waiting_car_for_order():
			if not _has_active_task_for_target(delivery_station, EmployeeTask.TaskType.SERVE_DRIVETHRU):
				var pos = delivery_station.global_position + Vector3(-0.8, 0.0, 0.0)
				_add_task(EmployeeTask.TaskType.SERVE_DRIVETHRU, delivery_station, pos, 2)

	# 3. PRIORIDADE 3: Mesas Sujas e Desocupadas
	var table_mgr = tree.root.find_child("TableManager", true, false)
	var tables = table_mgr.tables if table_mgr else tree.get_nodes_in_group("tables")
	for t in tables:
		var table = t as RestaurantTable
		if table and is_instance_valid(table) and table.table_state == RestaurantTable.TableState.DIRTY and table.seated_customers.is_empty():
			if not _has_active_task_for_target(table, EmployeeTask.TaskType.CLEAN_TABLE):
				var pos = table.global_position + Vector3(0.0, 0.0, 0.75)
	...
	for p in puddles:
		...
		if puddle and is_instance_valid(puddle) and puddle.get("is_dirty") != false:
	...
	for s in spots:
		...
		if spot and is_instance_valid(spot) and spot.get("is_dirty") != false:
	...
	for st in dirty_stations:
		...
		if station and is_instance_valid(station) and station.get("is_dirty") == true:
